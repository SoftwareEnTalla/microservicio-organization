...
@ApiTags("Seniority Query")
@UseGuards(SeniorityAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("senioritys/query")
export class SeniorityQueryController {
  #logger = new Logger(SeniorityQueryController.name);

  constructor(private readonly service: SeniorityQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all seniority with optional pagination" })
  @ApiResponse({ status: 200, type: SenioritysResponse })
  @ApiQuery({ name: "options", required: false, type: SeniorityDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Seniority>    
  ): Promise<SenioritysResponse<Seniority>> {
    try {
     
      const senioritys = await this.service.findAll(options);
      logger.info("Retrieving all seniority");
      return senioritys;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find seniority by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter seniority', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: SenioritysResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Seniority no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find senioritys with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Seniority>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all senioritys" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count senioritys with conditions" })
  @ApiResponse({ status: 200, type: SenioritysResponse<Seniority> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Senioritys no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one seniority with conditions" })
  ...
  @ApiOperation({ summary: "Find one seniority or return error" })
  @ApiResponse({ status: 200, type: SeniorityResponse<Seniority> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Seniority no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get seniority by ID" })
  @ApiResponse({ status: 200, type: SeniorityResponse<Seniority> })
  @ApiResponse({ status: 404, description: "Seniority not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the seniority to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<SeniorityResponse<Seniority>> {
    try {
      const seniority = await this.service.findOne({ where: { id } });
      if (!seniority) {
        throw new NotFoundException(
          "Seniority no encontrado para el id solicitado"
        );
      }
      return seniority;
    } catch (error) {
