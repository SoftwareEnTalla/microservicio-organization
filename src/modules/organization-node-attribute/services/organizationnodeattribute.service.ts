...
@Injectable()
export class OrganizationNodeAttributeService {
  constructor(private readonly repository: OrganizationNodeAttributeRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<OrganizationNodeAttribute[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<OrganizationNodeAttribute[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<OrganizationNodeAttribute[]> {
  ...
  async update(id: string, partialEntity: Partial<OrganizationNodeAttribute>): Promise<OrganizationNodeAttribute | null> {
