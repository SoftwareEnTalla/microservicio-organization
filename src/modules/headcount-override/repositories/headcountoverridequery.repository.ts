...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class HeadcountOverrideQueryRepository {

    //Constructor del repositorio de datos: HeadcountOverrideQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination HeadcountOverride on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[HeadcountOverride[], number]> {
      logger.info('Ready to findAndCount HeadcountOverride on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of HeadcountOverride on repository:',result);
      return result;
    }
    ...
    client: LoggerClient.getInstance()
      ...
      .get(HeadcountOverrideRepository.name),
        ...
        async findOne(options?: Record<string, any>): Promise<HeadcountOverride | null> {
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) HeadcountOverride:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<HeadcountOverride> = options as FindOptionsWhere<HeadcountOverride>;
