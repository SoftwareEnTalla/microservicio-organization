...
import { OrganizationDto } from "../dtos/all-dto";
...
@ApiTags("Organization Query")
@UseGuards(OrganizationAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("organizations/query")
export class OrganizationQueryController {
  #logger = new Logger(OrganizationQueryController.name);

  constructor(private readonly service: OrganizationQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all organization with optional pagination" })
  @ApiResponse({ status: 200, type: OrganizationsResponse })
  @ApiQuery({ name: "options", required: false, type: OrganizationDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Organization>    
  ): Promise<OrganizationsResponse<Organization>> {
    try {
     
      const organizations = await this.service.findAll(options);
      logger.info("Retrieving all organization");
  ...
  @ApiOperation({ summary: "Find organization by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter organization', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: OrganizationsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Organization no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find organizations with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Organization>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all organizations" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count organizations with conditions" })
  @ApiResponse({ status: 200, type: OrganizationsResponse<Organization> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one organization with conditions" })
  ...
  @ApiOperation({ summary: "Find one organization or return error" })
  @ApiResponse({ status: 200, type: OrganizationResponse<Organization> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the organization to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<OrganizationResponse<Organization>> {
    try {
      const organization = await this.service.findOne({ where: { id } });
      if (!organization) {
        throw new NotFoundException(
