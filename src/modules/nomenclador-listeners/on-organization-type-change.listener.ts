 ...
 * Licencia: MIT
 *
 * OnOrganizationTypeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `organization-type` (owner: organization). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'organization-type';
...
const TOPICS: string[] = [
  ...
  'organization-type-deleted',
];

@Injectable()
export class OnOrganizationTypeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnOrganizationTypeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnOrganizationTypeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnOrganizationTypeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onOrganizationTypeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
