 ...
 * Licencia: MIT
 *
 * OnOrganizationNodeStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `organization-node-status` (owner: organization). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'organization-node-status';
...
const TOPICS: string[] = [
  'organization-node-status-created',
  'organization-node-status-updated',
  'organization-node-status-deleted',
];

@Injectable()
export class OnOrganizationNodeStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnOrganizationNodeStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnOrganizationNodeStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnOrganizationNodeStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onOrganizationNodeStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
