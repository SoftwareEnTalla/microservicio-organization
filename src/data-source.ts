...
export const AppDataSource = new DataSource({
  ...
  database: process.env.DB_NAME || "organization-service",
...
async function checkPostgreSQLExtensions() {
  ...
  const organization = await pool.connect();

  try {
    for (const ext of REQUIRED_EXTENSIONS) {
      const res = await organization.query(
      ...
      if (res.rows.length === 0) {
        ...
        await organization.query('CREATE EXTENSION IF NOT EXISTS "' + ext + '"');
      }
    }
  } finally {
    await organization.release();
    await pool.end();
  }
}
