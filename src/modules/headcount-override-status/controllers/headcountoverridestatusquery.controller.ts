...
import { HeadcountOverrideStatusDto } from "../dtos/all-dto";
...
@ApiTags("HeadcountOverrideStatus Query")
@UseGuards(HeadcountOverrideStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("headcountoverridestatuss/query")
export class HeadcountOverrideStatusQueryController {
  #logger = new Logger(HeadcountOverrideStatusQueryController.name);

  constructor(private readonly service: HeadcountOverrideStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all headcountoverridestatus with optional pagination" })
  @ApiResponse({ status: 200, type: HeadcountOverrideStatussResponse })
  @ApiQuery({ name: "options", required: false, type: HeadcountOverrideStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<HeadcountOverrideStatus>    
  ): Promise<HeadcountOverrideStatussResponse<HeadcountOverrideStatus>> {
    try {
     
      const headcountoverridestatuss = await this.service.findAll(options);
      logger.info("Retrieving all headcountoverridestatus");
  ...
  @ApiOperation({ summary: "Find headcountoverridestatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter headcountoverridestatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: HeadcountOverrideStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "HeadcountOverrideStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find headcountoverridestatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<HeadcountOverrideStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all headcountoverridestatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count headcountoverridestatuss with conditions" })
  @ApiResponse({ status: 200, type: HeadcountOverrideStatussResponse<HeadcountOverrideStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one headcountoverridestatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one headcountoverridestatus or return error" })
  @ApiResponse({ status: 200, type: HeadcountOverrideStatusResponse<HeadcountOverrideStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get headcountoverridestatus by ID" })
  @ApiResponse({ status: 200, type: HeadcountOverrideStatusResponse<HeadcountOverrideStatus> })
  @ApiResponse({ status: 404, description: "HeadcountOverrideStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the headcountoverridestatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<HeadcountOverrideStatusResponse<HeadcountOverrideStatus>> {
    try {
      const headcountoverridestatus = await this.service.findOne({ where: { id } });
      if (!headcountoverridestatus) {
        throw new NotFoundException(
          "HeadcountOverrideStatus no encontrado para el id solicitado"
        );
      }
      return headcountoverridestatus;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
