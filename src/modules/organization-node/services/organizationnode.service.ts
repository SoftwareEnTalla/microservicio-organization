...
@Injectable()
export class OrganizationNodeService {
  constructor(private readonly repository: OrganizationNodeRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<OrganizationNode[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<OrganizationNode[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<OrganizationNode[]> {
  ...
  async update(id: string, partialEntity: Partial<OrganizationNode>): Promise<OrganizationNode | null> {
