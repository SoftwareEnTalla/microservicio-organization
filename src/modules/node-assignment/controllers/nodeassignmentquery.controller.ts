...
import { NodeAssignmentDto } from "../dtos/all-dto";
...
@ApiTags("NodeAssignment Query")
@UseGuards(NodeAssignmentAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("nodeassignments/query")
export class NodeAssignmentQueryController {
  #logger = new Logger(NodeAssignmentQueryController.name);

  constructor(private readonly service: NodeAssignmentQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all nodeassignment with optional pagination" })
  @ApiResponse({ status: 200, type: NodeAssignmentsResponse })
  @ApiQuery({ name: "options", required: false, type: NodeAssignmentDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<NodeAssignment>    
  ): Promise<NodeAssignmentsResponse<NodeAssignment>> {
    try {
     
      const nodeassignments = await this.service.findAll(options);
      logger.info("Retrieving all nodeassignment");
  ...
  @ApiOperation({ summary: "Find nodeassignment by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter nodeassignment', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: NodeAssignmentsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "NodeAssignment no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find nodeassignments with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<NodeAssignment>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all nodeassignments" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count nodeassignments with conditions" })
  @ApiResponse({ status: 200, type: NodeAssignmentsResponse<NodeAssignment> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades NodeAssignments no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one nodeassignment with conditions" })
  ...
  @ApiOperation({ summary: "Find one nodeassignment or return error" })
  @ApiResponse({ status: 200, type: NodeAssignmentResponse<NodeAssignment> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad NodeAssignment no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get nodeassignment by ID" })
  @ApiResponse({ status: 200, type: NodeAssignmentResponse<NodeAssignment> })
  @ApiResponse({ status: 404, description: "NodeAssignment not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the nodeassignment to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<NodeAssignmentResponse<NodeAssignment>> {
    try {
      const nodeassignment = await this.service.findOne({ where: { id } });
      if (!nodeassignment) {
        throw new NotFoundException(
          "NodeAssignment no encontrado para el id solicitado"
        );
      }
