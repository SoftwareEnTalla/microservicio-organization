...
@Injectable()
export class OrganizationNodeQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(OrganizationNodeQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: OrganizationNodeQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${OrganizationNode.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const organizationnodes = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de organizationnodes obtenido con éxito",
  ...
  async findById(id: string): Promise<OrganizationNodeResponse<OrganizationNode>> {
    try {
      const organizationnode = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "OrganizationNodes no encontrados para la propiedad y valor especificado"
        );
      // Devolver organizationnode
      return {
        ok: true,
        message: "OrganizationNodes obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<OrganizationNodeResponse<OrganizationNode>> {
    try {
