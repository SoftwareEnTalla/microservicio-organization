...
import { VacancyStatusDto } from "../dtos/all-dto";
...
@ApiTags("VacancyStatus Query")
@UseGuards(VacancyStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("vacancystatuss/query")
export class VacancyStatusQueryController {
  #logger = new Logger(VacancyStatusQueryController.name);

  constructor(private readonly service: VacancyStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all vacancystatus with optional pagination" })
  @ApiResponse({ status: 200, type: VacancyStatussResponse })
  @ApiQuery({ name: "options", required: false, type: VacancyStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<VacancyStatus>    
  ): Promise<VacancyStatussResponse<VacancyStatus>> {
    try {
     
      const vacancystatuss = await this.service.findAll(options);
      logger.info("Retrieving all vacancystatus");
  ...
  @ApiOperation({ summary: "Find vacancystatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter vacancystatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: VacancyStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "VacancyStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find vacancystatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<VacancyStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all vacancystatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count vacancystatuss with conditions" })
  @ApiResponse({ status: 200, type: VacancyStatussResponse<VacancyStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one vacancystatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one vacancystatus or return error" })
  @ApiResponse({ status: 200, type: VacancyStatusResponse<VacancyStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get vacancystatus by ID" })
  @ApiResponse({ status: 200, type: VacancyStatusResponse<VacancyStatus> })
  @ApiResponse({ status: 404, description: "VacancyStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the vacancystatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<VacancyStatusResponse<VacancyStatus>> {
    try {
      const vacancystatus = await this.service.findOne({ where: { id } });
      if (!vacancystatus) {
        throw new NotFoundException(
