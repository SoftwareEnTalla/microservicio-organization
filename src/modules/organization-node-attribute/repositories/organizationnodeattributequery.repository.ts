...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class OrganizationNodeAttributeQueryRepository {

    //Constructor del repositorio de datos: OrganizationNodeAttributeQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination OrganizationNodeAttribute on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[OrganizationNodeAttribute[], number]> {
      logger.info('Ready to findAndCount OrganizationNodeAttribute on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of OrganizationNodeAttribute on repository:',result);
      return result;
    }
    ...
    client: LoggerClient.getInstance()
      ...
      .get(OrganizationNodeAttributeRepository.name),
        ...
        async findOne(options?: Record<string, any>): Promise<OrganizationNodeAttribute | null> {
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) OrganizationNodeAttribute:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<OrganizationNodeAttribute> = options as FindOptionsWhere<OrganizationNodeAttribute>;
