...
import { HeadcountOverrideModeDto } from "../dtos/all-dto";
...
@ApiTags("HeadcountOverrideMode Query")
@UseGuards(HeadcountOverrideModeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("headcountoverridemodes/query")
export class HeadcountOverrideModeQueryController {
  #logger = new Logger(HeadcountOverrideModeQueryController.name);

  constructor(private readonly service: HeadcountOverrideModeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all headcountoverridemode with optional pagination" })
  @ApiResponse({ status: 200, type: HeadcountOverrideModesResponse })
  @ApiQuery({ name: "options", required: false, type: HeadcountOverrideModeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<HeadcountOverrideMode>    
  ): Promise<HeadcountOverrideModesResponse<HeadcountOverrideMode>> {
    try {
     
      const headcountoverridemodes = await this.service.findAll(options);
      logger.info("Retrieving all headcountoverridemode");
  ...
  @ApiOperation({ summary: "Find headcountoverridemode by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter headcountoverridemode', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: HeadcountOverrideModesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "HeadcountOverrideMode no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find headcountoverridemodes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<HeadcountOverrideMode>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all headcountoverridemodes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count headcountoverridemodes with conditions" })
  @ApiResponse({ status: 200, type: HeadcountOverrideModesResponse<HeadcountOverrideMode> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one headcountoverridemode with conditions" })
  ...
  @ApiOperation({ summary: "Find one headcountoverridemode or return error" })
  @ApiResponse({ status: 200, type: HeadcountOverrideModeResponse<HeadcountOverrideMode> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get headcountoverridemode by ID" })
  @ApiResponse({ status: 200, type: HeadcountOverrideModeResponse<HeadcountOverrideMode> })
  @ApiResponse({ status: 404, description: "HeadcountOverrideMode not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the headcountoverridemode to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<HeadcountOverrideModeResponse<HeadcountOverrideMode>> {
    try {
      const headcountoverridemode = await this.service.findOne({ where: { id } });
      if (!headcountoverridemode) {
        throw new NotFoundException(
          "HeadcountOverrideMode no encontrado para el id solicitado"
        );
      }
      return headcountoverridemode;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
