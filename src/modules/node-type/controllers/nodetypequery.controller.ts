...
@ApiTags("NodeType Query")
@UseGuards(NodeTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("nodetypes/query")
export class NodeTypeQueryController {
  #logger = new Logger(NodeTypeQueryController.name);

  constructor(private readonly service: NodeTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all nodetype with optional pagination" })
  @ApiResponse({ status: 200, type: NodeTypesResponse })
  @ApiQuery({ name: "options", required: false, type: NodeTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<NodeType>    
  ): Promise<NodeTypesResponse<NodeType>> {
    try {
     
      const nodetypes = await this.service.findAll(options);
      logger.info("Retrieving all nodetype");
      return nodetypes;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find nodetype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter nodetype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: NodeTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "NodeType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find nodetypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<NodeType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all nodetypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count nodetypes with conditions" })
  @ApiResponse({ status: 200, type: NodeTypesResponse<NodeType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades NodeTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one nodetype with conditions" })
  ...
  @ApiOperation({ summary: "Find one nodetype or return error" })
  @ApiResponse({ status: 200, type: NodeTypeResponse<NodeType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad NodeType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get nodetype by ID" })
  @ApiResponse({ status: 200, type: NodeTypeResponse<NodeType> })
  @ApiResponse({ status: 404, description: "NodeType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the nodetype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<NodeTypeResponse<NodeType>> {
    try {
      const nodetype = await this.service.findOne({ where: { id } });
      if (!nodetype) {
        throw new NotFoundException(
          "NodeType no encontrado para el id solicitado"
        );
      }
      return nodetype;
    } catch (error) {
