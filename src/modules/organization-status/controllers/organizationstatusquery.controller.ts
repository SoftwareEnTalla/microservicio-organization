...
@ApiTags("OrganizationStatus Query")
@UseGuards(OrganizationStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("organizationstatuss/query")
export class OrganizationStatusQueryController {
  #logger = new Logger(OrganizationStatusQueryController.name);

  constructor(private readonly service: OrganizationStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all organizationstatus with optional pagination" })
  @ApiResponse({ status: 200, type: OrganizationStatussResponse })
  @ApiQuery({ name: "options", required: false, type: OrganizationStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<OrganizationStatus>    
  ): Promise<OrganizationStatussResponse<OrganizationStatus>> {
    try {
     
      const organizationstatuss = await this.service.findAll(options);
      logger.info("Retrieving all organizationstatus");
  ...
  @ApiOperation({ summary: "Find organizationstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter organizationstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: OrganizationStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "OrganizationStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find organizationstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<OrganizationStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all organizationstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count organizationstatuss with conditions" })
  @ApiResponse({ status: 200, type: OrganizationStatussResponse<OrganizationStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one organizationstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one organizationstatus or return error" })
  @ApiResponse({ status: 200, type: OrganizationStatusResponse<OrganizationStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get organizationstatus by ID" })
  @ApiResponse({ status: 200, type: OrganizationStatusResponse<OrganizationStatus> })
  @ApiResponse({ status: 404, description: "OrganizationStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the organizationstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<OrganizationStatusResponse<OrganizationStatus>> {
    try {
      const organizationstatus = await this.service.findOne({ where: { id } });
