...
import { OrganizationElementTypeDto } from "../dtos/all-dto";
...
@ApiTags("OrganizationElementType Query")
@UseGuards(OrganizationElementTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("organizationelementtypes/query")
export class OrganizationElementTypeQueryController {
  #logger = new Logger(OrganizationElementTypeQueryController.name);

  constructor(private readonly service: OrganizationElementTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all organizationelementtype with optional pagination" })
  @ApiResponse({ status: 200, type: OrganizationElementTypesResponse })
  @ApiQuery({ name: "options", required: false, type: OrganizationElementTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<OrganizationElementType>    
  ): Promise<OrganizationElementTypesResponse<OrganizationElementType>> {
    try {
     
      const organizationelementtypes = await this.service.findAll(options);
      logger.info("Retrieving all organizationelementtype");
  ...
  @ApiOperation({ summary: "Find organizationelementtype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter organizationelementtype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: OrganizationElementTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "OrganizationElementType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find organizationelementtypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<OrganizationElementType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all organizationelementtypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count organizationelementtypes with conditions" })
  @ApiResponse({ status: 200, type: OrganizationElementTypesResponse<OrganizationElementType> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one organizationelementtype with conditions" })
  ...
  @ApiOperation({ summary: "Find one organizationelementtype or return error" })
  @ApiResponse({ status: 200, type: OrganizationElementTypeResponse<OrganizationElementType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get organizationelementtype by ID" })
  @ApiResponse({ status: 200, type: OrganizationElementTypeResponse<OrganizationElementType> })
  @ApiResponse({ status: 404, description: "OrganizationElementType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the organizationelementtype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<OrganizationElementTypeResponse<OrganizationElementType>> {
    try {
      const organizationelementtype = await this.service.findOne({ where: { id } });
      if (!organizationelementtype) {
        throw new NotFoundException(
          "OrganizationElementType no encontrado para el id solicitado"
        );
      }
      return organizationelementtype;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
