 ...
 * Licencia: MIT
 *
 * OnNodeAssignmentActionChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `node-assignment-action` (owner: organization). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'node-assignment-action';
...
const TOPICS: string[] = [
  'node-assignment-action-created',
  'node-assignment-action-updated',
  'node-assignment-action-deleted',
];

@Injectable()
export class OnNodeAssignmentActionChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnNodeAssignmentActionChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnNodeAssignmentActionChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnNodeAssignmentActionChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onNodeAssignmentActionChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
