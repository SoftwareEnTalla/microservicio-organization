...
@ApiTags("NodeAssignmentAction Query")
@UseGuards(NodeAssignmentActionAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("nodeassignmentactions/query")
export class NodeAssignmentActionQueryController {
  #logger = new Logger(NodeAssignmentActionQueryController.name);

  constructor(private readonly service: NodeAssignmentActionQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all nodeassignmentaction with optional pagination" })
  @ApiResponse({ status: 200, type: NodeAssignmentActionsResponse })
  @ApiQuery({ name: "options", required: false, type: NodeAssignmentActionDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<NodeAssignmentAction>    
  ): Promise<NodeAssignmentActionsResponse<NodeAssignmentAction>> {
    try {
     
      const nodeassignmentactions = await this.service.findAll(options);
      logger.info("Retrieving all nodeassignmentaction");
  ...
  @ApiOperation({ summary: "Find nodeassignmentaction by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter nodeassignmentaction', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: NodeAssignmentActionsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "NodeAssignmentAction no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find nodeassignmentactions with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<NodeAssignmentAction>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all nodeassignmentactions" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count nodeassignmentactions with conditions" })
  @ApiResponse({ status: 200, type: NodeAssignmentActionsResponse<NodeAssignmentAction> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one nodeassignmentaction with conditions" })
  ...
  @ApiOperation({ summary: "Find one nodeassignmentaction or return error" })
  @ApiResponse({ status: 200, type: NodeAssignmentActionResponse<NodeAssignmentAction> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get nodeassignmentaction by ID" })
  @ApiResponse({ status: 200, type: NodeAssignmentActionResponse<NodeAssignmentAction> })
  @ApiResponse({ status: 404, description: "NodeAssignmentAction not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the nodeassignmentaction to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<NodeAssignmentActionResponse<NodeAssignmentAction>> {
    try {
      const nodeassignmentaction = await this.service.findOne({ where: { id } });
      if (!nodeassignmentaction) {
        throw new NotFoundException(
          "NodeAssignmentAction no encontrado para el id solicitado"
        );
      }
      return nodeassignmentaction;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
