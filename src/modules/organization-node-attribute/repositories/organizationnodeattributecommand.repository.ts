...
import { OrganizationNodeAttributeQueryRepository } from './organizationnodeattributequery.repository';
import { generateCacheKey } from 'src/utils/functions';
import { Cacheable } from '../decorators/cache.decorator';
import {OrganizationNodeAttributeRepository} from './organizationnodeattribute.repository';
...
export class OrganizationNodeAttributeCommandRepository implements IEventHandler<BaseEvent>{
  ...
  //Constructor del repositorio de datos: OrganizationNodeAttributeCommandRepository
  constructor(
    @InjectRepository(OrganizationNodeAttribute)
    private readonly repository: Repository<OrganizationNodeAttribute>,
    private readonly organizationnodeattributeRepository: OrganizationNodeAttributeQueryRepository,
  ...
  async handle(event: any) {
    ...
    logger.info('Ready to handle OrganizationNodeAttribute event on repository:', event);
    switch (event.constructor.name) {
      case 'OrganizationNodeAttributeCreatedEvent':
        return await this.onOrganizationNodeAttributeCreated(event);
      case 'OrganizationNodeAttributeUpdatedEvent':
  ...
  @Cacheable({
    key: (args) => generateCacheKey<OrganizationNodeAttribute>('createOrganizationNodeAttribute', args[0], args[1]),
    ttl: 60,
  })
  private async onOrganizationNodeAttributeCreated(event: OrganizationNodeAttributeCreatedEvent) {
    logger.info('Ready to handle onOrganizationNodeAttributeCreated event on repository:', event);
    const entity = new OrganizationNodeAttribute();
    entity.id = event.aggregateId;
    Object.assign(entity, event.payload.instance);
    // Asegurar que el tipo discriminador esté establecido
    if (!entity.type) {
      entity.type = 'organizationnodeattribute';
    }
  ...
  @Cacheable({
    key: (args) => generateCacheKey<OrganizationNodeAttribute>('updateOrganizationNodeAttribute', args[0], args[1]),
    ttl: 60,
  })
  private async onOrganizationNodeAttributeUpdated(event: OrganizationNodeAttributeUpdatedEvent) {
    logger.info('Ready to handle onOrganizationNodeAttributeUpdated event on repository:', event);
  ...
  @Cacheable({
    key: (args) => generateCacheKey<OrganizationNodeAttribute>('deleteOrganizationNodeAttribute', args[0], args[1]),
    ttl: 60,
  })
  private async onOrganizationNodeAttributeDeleted(event: OrganizationNodeAttributeDeletedEvent) {
    logger.info('Ready to handle onOrganizationNodeAttributeDeleted event on repository:', event);
  ...
  async create(entity: OrganizationNodeAttribute): Promise<OrganizationNodeAttribute> {
    ...
    if (this.shouldPublishEvent()) {
      const event = new OrganizationNodeAttributeCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: OrganizationNodeAttribute[]): Promise<OrganizationNodeAttribute[]> {
    ...
    if (this.shouldPublishEvent()) {
      const events = result.map((el) => new OrganizationNodeAttributeCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    let instance=await this.organizationnodeattributeRepository.findById(id);
    logger.info('Updated instance of OrganizationNodeAttribute with id: ${id} was finded on repository:', instance);
    
    if(instance && this.shouldPublishEvent()) {
      logger.info('Ready to publish or fire event OrganizationNodeAttributeUpdatedEvent on repository:', instance);
      const event = new OrganizationNodeAttributeUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            const updateEvent = new OrganizationNodeAttributeUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     const entity = await this.organizationnodeattributeRepository.findOne({ id });
     if(!entity){
     ...
     if (this.shouldPublishEvent()) {
       logger.info('Ready to publish/fire OrganizationNodeAttributeDeletedEvent on repository:', result);
       const event = new OrganizationNodeAttributeDeletedEvent(id, {
        instance: entity,
        metadata: {
  ...
  async bulkDelete(ids: string[]): Promise<DeleteResult> {
    ...
    if (this.shouldPublishEvent()) {
      ...
      const deleteEvents = await Promise.all(ids.map(async (id) => {
          ...
          return new OrganizationNodeAttributeDeletedEvent(id, {
            instance: entity,
            metadata: {
              initiatedBy: entity.createdBy || 'system',
              correlationId: entity.id,
