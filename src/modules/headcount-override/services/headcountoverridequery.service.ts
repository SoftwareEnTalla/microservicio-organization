...
@Injectable()
export class HeadcountOverrideQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(HeadcountOverrideQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: HeadcountOverrideQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${HeadcountOverride.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const headcountoverrides = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de headcountoverrides obtenido con éxito",
        data: headcountoverrides,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
          headcountoverrides.length
        ),
        count: headcountoverrides.length,
      };
    } catch (error) {
      // Imprimir error
      logger.error(error);
  ...
  async findById(id: string): Promise<HeadcountOverrideResponse<HeadcountOverride>> {
    try {
      const headcountoverride = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "HeadcountOverrides no encontrados para la propiedad y valor especificado"
        );
      // Devolver headcountoverride
      return {
        ok: true,
        message: "HeadcountOverrides obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<HeadcountOverrideResponse<HeadcountOverride>> {
    try {
