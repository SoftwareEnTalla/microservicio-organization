 ...
 * Licencia: MIT
 *
 * OnHeadcountOverrideStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `headcount-override-status` (owner: organization). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'headcount-override-status';
const OWNER_BC = 'organization';
...
const TOPICS: string[] = [
  'headcount-override-status-created',
  'headcount-override-status-updated',
  'headcount-override-status-deleted',
];

@Injectable()
export class OnHeadcountOverrideStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnHeadcountOverrideStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnHeadcountOverrideStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnHeadcountOverrideStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onHeadcountOverrideStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
