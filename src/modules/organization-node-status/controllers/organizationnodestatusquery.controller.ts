...
@ApiTags("OrganizationNodeStatus Query")
@UseGuards(OrganizationNodeStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("organizationnodestatuss/query")
export class OrganizationNodeStatusQueryController {
  #logger = new Logger(OrganizationNodeStatusQueryController.name);

  constructor(private readonly service: OrganizationNodeStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all organizationnodestatus with optional pagination" })
  @ApiResponse({ status: 200, type: OrganizationNodeStatussResponse })
  @ApiQuery({ name: "options", required: false, type: OrganizationNodeStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<OrganizationNodeStatus>    
  ): Promise<OrganizationNodeStatussResponse<OrganizationNodeStatus>> {
    try {
     
      const organizationnodestatuss = await this.service.findAll(options);
      logger.info("Retrieving all organizationnodestatus");
  ...
  @ApiOperation({ summary: "Find organizationnodestatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter organizationnodestatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: OrganizationNodeStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "OrganizationNodeStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find organizationnodestatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<OrganizationNodeStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all organizationnodestatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count organizationnodestatuss with conditions" })
  @ApiResponse({ status: 200, type: OrganizationNodeStatussResponse<OrganizationNodeStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one organizationnodestatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one organizationnodestatus or return error" })
  @ApiResponse({ status: 200, type: OrganizationNodeStatusResponse<OrganizationNodeStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get organizationnodestatus by ID" })
  @ApiResponse({ status: 200, type: OrganizationNodeStatusResponse<OrganizationNodeStatus> })
  @ApiResponse({ status: 404, description: "OrganizationNodeStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the organizationnodestatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<OrganizationNodeStatusResponse<OrganizationNodeStatus>> {
    try {
      const organizationnodestatus = await this.service.findOne({ where: { id } });
      if (!organizationnodestatus) {
        throw new NotFoundException(
          "OrganizationNodeStatus no encontrado para el id solicitado"
        );
      }
      return organizationnodestatus;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
