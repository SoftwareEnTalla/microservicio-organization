...
@Injectable()
export class HeadcountOverrideService {
  constructor(private readonly repository: HeadcountOverrideRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<HeadcountOverride[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<HeadcountOverride[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<HeadcountOverride[]> {
  ...
  async update(id: string, partialEntity: Partial<HeadcountOverride>): Promise<HeadcountOverride | null> {
