...
import { OrganizationTypeDto } from "../dtos/all-dto";
...
@ApiTags("OrganizationType Query")
@UseGuards(OrganizationTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("organizationtypes/query")
export class OrganizationTypeQueryController {
  #logger = new Logger(OrganizationTypeQueryController.name);

  constructor(private readonly service: OrganizationTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all organizationtype with optional pagination" })
  @ApiResponse({ status: 200, type: OrganizationTypesResponse })
  @ApiQuery({ name: "options", required: false, type: OrganizationTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<OrganizationType>    
  ): Promise<OrganizationTypesResponse<OrganizationType>> {
    try {
     
      const organizationtypes = await this.service.findAll(options);
      logger.info("Retrieving all organizationtype");
  ...
  @ApiOperation({ summary: "Find organizationtype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter organizationtype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: OrganizationTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "OrganizationType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find organizationtypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<OrganizationType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all organizationtypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count organizationtypes with conditions" })
  @ApiResponse({ status: 200, type: OrganizationTypesResponse<OrganizationType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades OrganizationTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one organizationtype with conditions" })
  ...
  @ApiOperation({ summary: "Find one organizationtype or return error" })
  @ApiResponse({ status: 200, type: OrganizationTypeResponse<OrganizationType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad OrganizationType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get organizationtype by ID" })
  @ApiResponse({ status: 200, type: OrganizationTypeResponse<OrganizationType> })
  @ApiResponse({ status: 404, description: "OrganizationType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the organizationtype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<OrganizationTypeResponse<OrganizationType>> {
    try {
      const organizationtype = await this.service.findOne({ where: { id } });
      if (!organizationtype) {
        throw new NotFoundException(
          "OrganizationType no encontrado para el id solicitado"
        );
      }
