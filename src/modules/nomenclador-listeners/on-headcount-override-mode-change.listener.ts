 ...
 * Licencia: MIT
 *
 * OnHeadcountOverrideModeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `headcount-override-mode` (owner: organization). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'headcount-override-mode';
...
const TOPICS: string[] = [
  'headcount-override-mode-created',
  'headcount-override-mode-updated',
  'headcount-override-mode-deleted',
];

@Injectable()
export class OnHeadcountOverrideModeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnHeadcountOverrideModeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnHeadcountOverrideModeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnHeadcountOverrideModeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onHeadcountOverrideModeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
